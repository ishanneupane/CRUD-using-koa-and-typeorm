...
router.get("/:id", async (ctx: Context) => {
  const movieRepo: Repository<movieEntity> =ctx.state.db.getRepository(movieEntity);
  const movie = await movieRepo.findOne({where:{id:(ctx as any).params.id as string} } );  // Using type assertion here
  if (!movie) {
...
router.put("/:id", async (ctx: Koa.Context) => {
  const movieRepo: Repository<movieEntity> = ctx.state.db. getRepository(movieEntity);
  const movie = await movieRepo.findOne({where: {id:(ctx as any).params.id as string}});
  if (!movie) {
    ctx.throw(HttpStatus.StatusCodes.NOT_FOUND);
  }
